...
struct _Exiv2ExifDatumIterator {
    _Exiv2ExifDatumIterator(Exiv2::ExifMetadata::const_iterator i, Exiv2::ExifMetadata::const_iterator e)
        : it(i), end(e) {
    }
...
struct _Exiv2IptcDatumIterator {
    _Exiv2IptcDatumIterator(Exiv2::IptcMetadata::const_iterator i, Exiv2::IptcMetadata::const_iterator e)
        : it(i), end(e) {
    }
...
_Exiv2Error::_Exiv2Error(const Exiv2::Error &error)
    : code(error.code()), what(strdup(error.what())) {
}
...
void exiv2_image_set_icc_profile(Exiv2Image *img, const unsigned char *profile, long size, Exiv2Error **error) {
    try {
        Exiv2::DataBuf buf(profile, size);
        img->image->setIccProfile(buf);
        img->image->writeMetadata();
    } catch (Exiv2::Error &e) {
        if (error) {
            *error = new Exiv2Error(e);
